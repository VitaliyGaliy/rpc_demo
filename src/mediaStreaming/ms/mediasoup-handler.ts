...
export class MediasoupHandler extends EventEmitter implements IMediasoupApi {
  ...
  private readonly _mapRouterPipeTransports: Map<
    string,
    {local: PipeTransportData; remote: PipeTransportData}
  > = new Map();
  ...
  async [ACTION.CONSUME]({
    kind,
    rtpCapabilities,
    stream,
    transportId,
  }: ConsumeRequest): Promise<ConsumeResponse> {
  ...
  }: ProduceRequest): Promise<ProduceResponse> {
    const transport = this.getTransport(transportId);
  ...
  async [ACTION.SET_PREFERRED_LAYERS]({
    consumerId,
    layers,
  }: ConsumerPreferredLayers): Promise<void> {
    const consumer = this.getConsumer(consumerId);
  ...
  async [ACTION.SET_MAX_INCOMING_BITRATE]({
    transportId,
    bitrate,
  }: TransportBitrateData): Promise<void> {
    const transport = this.getTransport(transportId);
  ...
  async [ACTION.CREATE_PIPE_TRANSPORT](): Promise<PipeTransportData> {
    const transport = await this._router.createPipeTransport({
      listenIp: this._conf.webRtcTransport.listenIp,
      enableSctp: true,
  ...
  async [ACTION.CONNECT_PIPE_TRANSPORT]({
    pipeTransportId,
    ip,
    port,
    transportId,
  }: PipeTransportConnectData): Promise<void> {
    const transport = this.getTransport(transportId);
  ...
  async [ACTION.CONNECT_TRANSPORT]({
    transportId,
    dtlsParameters,
  }: ConnectTransportRequest): Promise<void> {
    const transport = this.getTransport(transportId);
  ...
  async [ACTION.STOP_RECORDING]({
    stream,
    kinds = ['audio', 'video'],
  }: StopRecordingRequest): Promise<void> {
  ...
  async [ACTION.START_RECORDING]({
    stream,
    filePath,
    kinds = ['audio', 'video'],
    wait = true,
  }: StartRecordingRequest): Promise<void> {
    ...
    for (const kind of kinds) {
      ...
      if (!this._recorders.get(id)) {
        ...
        if (producerId) {
          ...
          if (producer) {
            const {port, consumer} = await this.getStreamInputFromServer(
              producer,
            );
            if (!this._recorders.get(id)) {
              return;
  ...
  private setProducerListeners(producer: Producer) {
    ...
    producer.observer.on('close', () => {
      ...
      this._rtpIndexes
        ...
        .forEach((producerId, rtpIndex) => delete this._rtpIndexes[rtpIndex]);
    });
  }
  ...
  private async createPlainRtpTransport(
    comedia?: boolean,
    rtcpMux?: boolean,
  ...
  private async createPlainRtpProducer(
  ...
  private async producerIdByStream(
    stream: string,
    kind: MediaKind,
    wait: boolean = true,
    ...
    return this._producerIdByStream.get(
      MediasoupHandler.streamKindId(stream, kind),
    );
  }
  ...
  private async getStreamInputFromServer(
    producer: Producer,
  ): Promise<{port: number; consumer: Consumer}> {
  ...
  async startRestreaming({
    ...
    if (pipeTransportPair) {
      ...
      await Promise.all([
        this.connectPipeTransport({
          ...remote,
          transportId: local.pipeTransportId,
        }),
        api.connectPipeTransport({
          ...local,
    ...
    for (const kind of kinds) {
      ...
      if (producerId) {
        ...
        if (producer) {
          const localTransport = this._transports.get(local.pipeTransportId);
          if (localTransport) {
            const pipeConsumer: Consumer = await localTransport.consume({
              producerId: producer.id,
            });
            ...
            pipeConsumer.observer.on('close', () =>
              api.closeProducer({producerId: remoteProducerData.id}),
            );
            pipeConsumer.observer.on('pause', () =>
              api.pauseProducer({producerId: remoteProducerData.id}),
            );
            pipeConsumer.observer.on('resume', () =>
              api.resumeProducer({producerId: remoteProducerData.id}),
            );
          }
        }
      }
  ...
  async streamFile({
    ...
    stream,
    kinds = ['audio', 'video'],
  }: StreamFileRequest): Promise<void> {
    ...
    for (const kind of kinds) {
      ...
      tee.push(
        `[select=${kind.charAt(0)}:f=rtp:ssrc=${+ssrc}:payload_type=${
          this._conf.codecParameters[kind].payloadType
        }]rtp://127.0.0.1:${transport.tuple.localPort}?rtcpport=${
